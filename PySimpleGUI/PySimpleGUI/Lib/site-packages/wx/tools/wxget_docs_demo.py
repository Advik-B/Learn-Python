...
def get_paths_dict():
    ...
    pathdict['Docs_Name'] = f"wxPython-docs-{wx.VERSION_STRING}"
    ...
    pathdict['Demo_Name'] = f"wxPython-demo-{wx.VERSION_STRING}"
    pathdict['Ext'] = 'tar.gz'
    return pathdict
...
def docs_main(args=sys.argv):
    ...
    ensure_wx_app()
    result = 0
    print(f"Launch Docs for wxPython V{wx.VERSION_STRING}")
    pd = get_paths_dict()
    if location := get_item(
        pd['wxDocs'],
        pd['Docs_URL'],
        pd['Cache'],
        pd['Docs_Name'],
...
def demo_main(args=sys.argv):
    ...
    result = 0
    ensure_wx_app()
    print(f"Launch Demo for wxPython V{wx.VERSION_STRING}")
    pd = get_paths_dict()
    if location := get_item(
        pd['wxDemo'],
        pd['Demo_URL'],
        ...
        print(f"Demo starting as PID {pid} - may take a few seconds!")
    else:
